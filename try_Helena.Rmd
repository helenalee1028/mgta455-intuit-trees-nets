...
```{r}
decision_tree_pred = predict(decision_tree, pred_data = test, type = 'prob')
test$decision_tree_pred = decision_tree_pred
```

## Bagging
```{r}
bagging = bagging(formula = res1 ~  zip_bins + numords + dollars + last + version1 + owntaxprod + upgraded + zip801 + zip804, data = train, nbagg=25)
...
tuning_random_forest <- tuneRF(x = subset(train, select = columns),
              y = train$res1,
              ntreeTry = 500)
mtry_opt <- tuning_random_forest[,"mtry"][which.min(tuning_random_forest[,"OOBError"])]
```
...
vars_vec = c('decision_tree_pred','bagging_pred','random_forest_pred')
