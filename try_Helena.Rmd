...
```{r}
decision_tree = rpart(formula = res1 ~  zip_bins + numords + dollars + last + version1 + owntaxprod + upgraded + zip801 + zip804, data = train, method = 'class', control=rpart.control(minsplit=2, minbucket=1, cp=0.001))
```

```{r}
decision_tree_pred = predict(object = decision_tree, newdata = test, type = 'class')
