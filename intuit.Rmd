...
From profit and ROME comparisons we can see that the ensemble result of GBM and XGB perform the best in terms of expected profits. Though random forest has the highest ROME, it has higher risk of missing potential customers. Therefore, we favor the model with highest expected profits.

In lift and gain charts, all gradient based models perform quite similar while random forest performs the worst. In confusion matrix term, again emsemble_2m is one of the best models in terms of both AUC and TRP. 

Therefore, we use predicted probabilities in Ensemble_2m to determine whom to mail during wave-2 promotions.

```{r}
targets <- final_eval %>% 
  mutate(mailto_wave2 = ifelse(ensemble_2m > 2*breakeven_rate, "TRUE", "FALSE")) %>% 
