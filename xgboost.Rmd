...
```
## First tune general parameter in xgboost model--nrounds, eta and max_depth.
...
params %>% 
  top_n(auc, n = 5) 
```
From all the 5 best combinations with higher auc, we can decide on the best eta is 0.2. Also, the best nround would be something between 20 and 30. We then use this for further tuning. AS max_depth = 6 does better than max_depth = 7 or 8, we draw the conclusion that the best max_depth would be something smaller than 6. 


Second,we create a random search, we generate random combinations of the hyperparameters and to find the best for the models. We can afford to test parameters with wider range compared to grid search. We use randome search to narrow down parameter range and then put into gird search to get more accurate values.
```{r,eval = FALSE}
...
Then we do grid search for more accurate estimate.
```{r,eval = FALSE}
...
We trained our best model, make prediction on test set and generate profit estimation.
