...
bbb <- readr::read_rds(file.path(find_dropbox(), "MGTA455-2019/data/bbb.rds"))
...
## using caret to create a new training variable
## not required, just an example
training <- rep(0L, nrow(bbb))
set.seed(1234)
ind <- createDataPartition(bbb$buyer, p = .7 )[[1]]
training[ind] <- 1L
...
summary(result)
eval_dat$nn2r <- predict(result, bbb)$Prediction

## using the cv.nn function for cross validation
## use the big grid ...
# cv.nn(result, K = 10, size = 1:6, decay = seq(0, 1, 0.05))
## ... or the small grid as an example
cv.nn(result, K = 5, size = 1:3, decay = seq(0.05, 0.15, 0.05))
...
df_train_scaled <- scaledf(df_train, sf = 2)
str(df_train_scaled)
...
## on my mac - for more on custom evaluation functions
